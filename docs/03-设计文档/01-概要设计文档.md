...
**版本：3.1.0**
**日期：2026年8月15日**
**修订说明**：v3.1.0 新增崩溃自愈（owner 崩溃接管 + 奇数 write_seq 恢复）与集成产品化（find_package / pkg-config / FetchContent）。
（v3.0.0：纯 C11 重写，移除 C++ 类层次，所有模块改为 C 结构体 + 自由函数；API 层从 C++/C ABI 双轨改为纯 C 单头文件 (26 个 C 函数)；新增 check_timeouts 超时检测）
...
#### 关键特性：
- **无锁读取**：读端走 Seqlock 重试循环，读路径无锁、无阻塞
- **`peek()` 为单拷贝**：Seqlock 重试循环内拷贝到 `_Thread_local` 缓冲后返回其指针（同线程下次 `peek` 覆盖）；如需长期持有请用 `read_point()`
...
sequenceDiagram
    ...
    PM->>SM: Seqlock 重试循环内拷贝 PointData[1001]
...
| **通知机制** | 进程内回调（当前）／Unix Domain Socket（规划中） | 当前订阅为**进程内**回调，跨进程通知规划于 v3.3；UDS 本地高效、支持 fd 传递 |
| **配置格式** | YAML | 人类可读，支持注释 |
...
- **跨进程实例隔离**：不同 `instance_id` 对应不同段名（`/indurtdb_hvac`、`/indurtdb_lighting`），互不可见；
- **单进程单例**：同一进程内 `indurtdb_initialize()` 只能持有一个实例，切换实例须先 `indurtdb_shutdown()`；
...
5. **跨进程通知机制**（v3.3 规划）：当前 OSAL 仅有时间 + 共享内存两类能力，**无 `INotification` 接口**；实现跨进程通知需新增 OSAL 通知能力（方案待评审）
