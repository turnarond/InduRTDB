...
> ⚠️ **版本口径更新（2026-09-23）**：本文标题与正文中的 **v3.2.0** 指"node-server 集成"这一里程碑，但按最新评审决策，**v3.2 = 文档治理版本**，集成能力顺延。
> 特别地，**第 4 节「InduRTDB v3.2.0 新增 API（name→ID / 元数据查询）」已被决策 D3 取代**：按名查找改为 shm 内 name→id 索引，属 ABI 变更，排 **v4.0**；进程内查找表方案不再实施。
> 详见 [`06-开发规划/08-功能评审与下一步规划.md`](../06-开发规划/08-功能评审与下一步规划.md)。其余集成设计（存储适配器 / 数据模型映射 / 北向复用 / 持久化复用）仍有效。
