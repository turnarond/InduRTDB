...
> ⚠️ **历史快照文档（v3.0.0，2026-07-23）**
> 本文记录 v3.0.0 交付时的状态，**不代表当前实现**（当前为 v3.1.0）。当前行为以 [`01-产品白皮书.md`](../01-白皮书/01-产品白皮书.md)、[`03-设计文档/`](../03-设计文档/) 与 [`05-SDK手册/`](../05-SDK手册/) 为准；已知历史表述与现行实现的差异已在文中就地标注。
...
|----------|:----------:|:--------:|
| 单点读写 (4 类型) | Yes | Yes |
| peek | Yes (仅 C++，真零拷贝) | Yes (C 可用；**v3.1 起改为单拷贝到线程本地缓冲**) |
