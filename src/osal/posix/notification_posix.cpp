 ...
 * @brief POSIX通知机制实现（Unix Domain Socket）
 * @version 1.0.0
 * @date 2026-03-27
...
public:
    ...
    bool receive(void* data, std::size_t size, TimestampNs timeout_ns) override {
        ...
        if (timeout_ns > 0) {
            struct timeval tv;
            tv.tv_sec = timeout_ns / 1000000000ULL;
            tv.tv_usec = (timeout_ns % 1000000000ULL) / 1000ULL;
...
INotification* create_notification(const std::string& path, bool as_server) {
    auto* notification = new (std::nothrow) Notification(path, as_server);
    if (!notification) {
        return nullptr;
    }
...
extern "C" {
    indurtdb::osal::INotification* create_notification(const char* path, bool as_server) {
        return indurtdb::osal::posix::create_notification(path, as_server);
    }
    
    void destroy_notification(indurtdb::osal::INotification* notification) {
        indurtdb::osal::posix::destroy_notification(notification);
    }
}